  ...
  if (term.element) {
    ...
    const fixAllLayers = () => {
      ...
      if (textLayers.length > 0) {
        if (logDetailed) console.log(`确保${textLayers.length}个xterm-text-layer可见`);
      ...
      // 确保光标层可见且在最顶层
      if (cursorLayers.length > 0) {
        if (logDetailed) console.log(`确保${cursorLayers.length}个xterm-cursor-layer可见`);
        cursorLayers.forEach(layer => {
          if (layer instanceof HTMLElement) {
            layer.style.opacity = '1';
            layer.style.visibility = 'visible';
            layer.style.zIndex = '35'; // 提高z-index确保在文本层之上
            // 确保光标层不会被其他层遮挡
            layer.style.pointerEvents = 'none'; // 允许点击透过光标层
          }
        });
      }

      // 查找并修复xterm-viewport，确保正确的滚动行为
      const viewports = document.querySelectorAll('.xterm-viewport');
      if (viewports.length > 0 && logDetailed) {
        console.log(`确保${viewports.length}个xterm-viewport正确配置`);
        viewports.forEach(viewport => {
          if (viewport instanceof HTMLElement) {
            viewport.style.overflowY = 'auto';
            viewport.style.zIndex = '20';
            // 确保滚动区域正确显示
      ...
      // 查找并修复xterm-screen，确保内容正确对齐
      const screens = document.querySelectorAll('.xterm-screen');
      if (screens.length > 0 && logDetailed) {
        console.log(`确保${screens.length}个xterm-screen正确配置`);
    ...
    [100, 300, 600, 1000].forEach(delay => {
      setTimeout(() => {
        console.log(`渲染后${delay}ms自动修复层级`);
        fixAllLayers();
      }, delay);
    });
  }
